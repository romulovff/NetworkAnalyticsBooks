...
#Per category
ggplot(dt.books.categories, aes(x=categories, y=n_cat)) + geom_bar(stat='identity') + ggtitle('Books per Category') + xlab('Categories') + ylab('Number of Books')

#Per year
books.per.year <- unlist(table(dt.books2$published_year))
df.books.per.year <- data.frame(books.per.year)
ggplot(df.books.per.year,aes(x=Var1, y=Freq)) + geom_bar(stat='identity') + ggtitle('Books Published per Year') + xlab('Year') + ylab('Number of Books')

#Per average rating
ggplot(dt.books,aes(x=average_rating)) + geom_histogram() +  ggtitle('Books per Ranking') + xlab('Average Ranking') + ylab('Number of Books')

#Authors with more published books
dt.books.clean <- data.table(dt.books.clean)
dt.books.clean[,list(n_books = .N), by=authors][order(-n_books)][1:10]

#Authors with more ranking
...
```{r}
#GRAPHS - isto tem de vir depois do código que cria as classes e os avg ratings, que a Mónica fez
...
```{r   }
# Creating columns n_books, n_books_class
n.books <- vector()
for (author in dt.books$authors) {
  the_author <- dt.books[dt.books$authors %in% author, ]
